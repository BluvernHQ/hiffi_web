...
export default async function AppDownloadPage() {
  ...
  return (
    <div className="relative min-h-full bg-gradient-to-b from-white via-white to-red-50/50 text-foreground">
      <JsonLd data={softwareJsonLd} />
      <JsonLd data={faqJsonLd} />

      <AppDownloadChrome initialPlatform={initialPlatform}>
        <div className="relative z-[1]">
          <section className="relative mx-auto max-w-6xl border-t border-red-100 bg-white px-4 py-20 text-center sm:px-6 md:py-28 lg:px-8">
            <p
              aria-hidden
              className="pointer-events-none absolute left-1/2 top-8 -translate-x-1/2 select-none text-[clamp(4rem,12vw,9rem)] font-black uppercase tracking-[-0.04em] text-primary/[0.06]"
            >
              HIPHOP
            </p>
            <h2 className="font-mono text-[11px] font-semibold uppercase tracking-[0.28em] text-primary">What is Hiffi?</h2>
            <p className="mx-auto mt-6 max-w-3xl text-lg leading-relaxed text-foreground/80 md:text-xl">
          ...
          <section className="border-t border-red-100 bg-red-50/35 py-20 md:py-28">
            <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
              ...
              <div className="mt-10 grid gap-4 text-left sm:grid-cols-2 lg:gap-5 xl:grid-cols-3">
                {fanFeatures.map(({ icon: Icon, title, description }, idx) => (
                  <article
                    key={title}
                    className={`border-2 border-primary/20 p-5 md:p-6 ${idx % 3 === 1 ? "-rotate-[0.8deg] bg-white" : idx % 3 === 2 ? "rotate-[0.7deg] bg-red-50/50" : "bg-white"} shadow-[6px_6px_0_rgba(237,28,47,0.12)]`}
                  >
                    <div className="flex h-11 w-11 items-center justify-center border-2 border-primary/30 bg-primary text-primary-foreground">
                      <Icon className="h-5 w-5" strokeWidth={1.75} aria-hidden />
                    </div>
                    <h3 className="mt-4 text-base font-semibold uppercase tracking-tight text-foreground">{title}</h3>
          ...
          <section className="border-t border-red-100 bg-white py-20 md:py-28">
            <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
              ...
              <div className="mt-10 grid gap-4 text-left sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">
                {artistFeatures.map(({ icon: Icon, title, description }, idx) => (
                  <article
                    key={title}
                    className={`border-2 border-primary/20 p-5 md:p-6 ${idx % 2 === 0 ? "bg-white" : "bg-red-50/40"} shadow-[6px_6px_0_rgba(237,28,47,0.12)] ${idx === 1 ? "rotate-[0.8deg]" : idx === 2 ? "-rotate-[0.6deg]" : ""}`}
                  >
                    <div className="flex h-11 w-11 items-center justify-center border-2 border-primary/30 bg-primary text-primary-foreground">
                      <Icon className="h-5 w-5" strokeWidth={1.75} aria-hidden />
                    </div>
            ...
            <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
              ...
              <div className="mt-10 flex flex-wrap justify-center gap-3">
                {appFeaturePills.map((label) => (
                  <span
                    key={label}
                    className="inline-flex border-2 border-primary/20 bg-white px-4 py-2.5 text-sm font-semibold uppercase tracking-wide text-foreground shadow-[4px_4px_0_rgba(237,28,47,0.12)]"
                  >
                    {label}
                  </span>
                ))}
          ...
          <section className="border-t border-red-100 bg-white py-20 md:py-24">
            <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
              <h2 className="font-mono text-[11px] font-semibold uppercase tracking-[0.28em] text-primary">
              ...
              <Link
                ...
                className="mt-8 inline-flex border-2 border-primary bg-primary px-8 py-3 text-sm font-semibold uppercase tracking-wide text-primary-foreground shadow-[6px_6px_0_rgba(237,28,47,0.25)] transition-all hover:translate-x-[1px] hover:translate-y-[1px] hover:bg-primary/90 hover:shadow-[5px_5px_0_rgba(237,28,47,0.35)]"
              >
                Open FAQ
              </Link>
            </div>
