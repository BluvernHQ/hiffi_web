...
function loadStateFromStorage(
  filter: FilterType,
): { videos: any[]; offset: number; hasMore: boolean } | null {
...
function saveScrollPosition() {
  ...
  try {
    ...
    if (mainContent)
      sessionStorage.setItem(SCROLL_KEY, String(mainContent.scrollTop));
  } catch (error) {
...
export default function HomeClient({ initialVideos = [] }: HomeClientProps) {
    ...
    try {
      ...
      const enhancedVideos = videosArray.map((video: any) => {
        ...
        const isCurrentUserVideo =
          userData?.username && videoUsername === userData.username;
        if (
          isCurrentUserVideo &&
          !video.user_profile_picture &&
          (userData.profile_picture || userData.image)
        ) {
          video.user_profile_picture =
            userData.profile_picture || userData.image;
      ...
      if (currentOffset === 0) {
        ...
        setTimeout(
          () =>
            saveStateToStorage(
              ...
              enhancedVideos.length === VIDEOS_PER_PAGE,
              currentFilter,
            ),
          0,
        );
        ...
        setVideos((prev) => {
          const existingIds = new Set(
            prev.map((v) => (v as any).videoId || (v as any).video_id),
          );
          const newVideos = enhancedVideos.filter(
            (v) => !existingIds.has((v as any).videoId || (v as any).video_id),
          );
          ...
          setTimeout(
            () =>
              saveStateToStorage(
                updatedVideos,
                newOffset,
                videosArray.length === VIDEOS_PER_PAGE,
                currentFilter,
              ),
            0,
          );
  ...
  useEffect(() => {
    try {
      ...
      if (
        restoredState &&
        (!savedStateSeed || savedStateSeed === currentSeed)
      ) {
        if (typeof window !== "undefined") {
          try {
            const stored = sessionStorage.getItem(STORAGE_KEY);
      ...
      <div className="w-full">
        ...
        <VideoGrid
          ...
          onVideoDeleted={(videoId) => {
            setVideos((prev) =>
              prev.filter(
                (v) => ((v as any).videoId || (v as any).video_id) !== videoId,
              ),
            );
          }}
        />
