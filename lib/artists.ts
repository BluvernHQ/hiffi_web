...
/** Accessible / SEO alt text for artist profile photos. */
export function artistProfilePhotoAlt(name: string): string {
  const trimmed = name.trim() || "Artist"
  return `${trimmed} — hip-hop artist profile photo`
}
