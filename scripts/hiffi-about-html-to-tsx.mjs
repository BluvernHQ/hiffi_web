...
if (!inputPath || !componentName) {
  console.error("Usage: node scripts/hiffi-about-html-to-tsx.mjs <input.html> <ComponentName> [section|footer]");
  process.exit(1);
}
...
const fileName =
  componentName === "HiffiAboutFooter"
    ? "footer.tsx"
    : componentName
        .replace(/^HiffiAbout/, "")
        .replace(/([A-Z])/g, "-$1")
...
const outDir = join(dirname(fileURLToPath(import.meta.url)), "../app/(main)/about/components/hiffi-about/sections");
...
export default function ${componentName}() {
  const ref = useHiffiAboutSectionInit("${className}");
