...
/** Workers CDN base from /top-artists/runtime-config (NEXT_PUBLIC_WORKERS_URL). */
...
function resolveArtistImageUrl(raw) {
  ...
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    try {
      ...
      if (u.origin === window.location.origin && u.pathname.startsWith("/proxy/profile-picture/")) {
        ...
        return base && path ? `${base}/${path}${u.search}` : null;
      }
  ...
  return base && clean ? `${base}/${clean}` : null;
}
...
function artistAvatarHTML(artist) {
  ...
  if (photo) {
    return `<i class="artist-avatar has-photo" style="--avatar-accent:${accent}" data-username="${escapeAttr(artist.username || "")}"><img src="${escapeAttr(photo)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.closest('.artist-avatar')?.classList.remove('has-photo');this.remove()"></i>`;
  }
