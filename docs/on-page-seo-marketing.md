...
**Production requirement:** `NEXT_PUBLIC_ENV=prod` — non-prod environments serve `robots.txt` with `Disallow: /` for generic crawlers (so staging is not indexed), while named AI bots are explicitly `Allow: /`.
